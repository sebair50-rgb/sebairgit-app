...
.grid-bg{
  background-image:linear-gradient(rgba(48,54,61,.3) 1px,transparent 1px),linear-gradient(90deg,rgba(48,54,61,.3) 1px,transparent 1px);
...
.step-dot{width:33px;height:33px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;font-family:var(--mono);transition:all .3s}
.step-dot.p{background:var(--surf3);border:2px solid var(--border);color:var(--dim)}
.step-dot.a{background:var(--green-gl);border:2px solid var(--green);color:var(--green-lt)}
.step-dot.d{background:var(--green);border:2px solid var(--green);color:#fff}
.step-dot.e{background:var(--red-dim);border:2px solid var(--red);color:var(--red)}
...
/* Error banner */
.err-banner{background:var(--red-dim);border-bottom:1px solid rgba(248,81,73,.3);padding:10px 20px;display:flex;align-items:center;justify-content:space-between;gap:12px;animation:fadeUp .3s ease}
...
function buildOAuthURL() {
  ...
  if (!clientId) return '/api/auth/login'           // backend fallback
...
export default function App() {
  ...
  /* ── Auth resolution — runs once on mount ───────────────────────────────── */
  useEffect(() => {
    ...
    // Always clean the URL immediately so refreshing doesn't re-process
    if (authOk || errParam) {
      window.history.replaceState({}, '', '/')
    }

    /* Path A ── Fresh OAuth redirect with user data in URL */
    if (authOk && rawU) {
      try {
        // Backend sends standard base64 — decode with atob()
        const json    = atob(decodeURIComponent(rawU))
        const decoded = JSON.parse(json)
        if (decoded && decoded.login) {
          setUser(decoded)
          try { localStorage.setItem('sg_user', JSON.stringify(decoded)) } catch {}
          setAuthLoading(false)
          return
        }
      } catch (e) {
        console.error('[auth] Failed to decode user from URL:', e)
        setAuthError('Login failed — could not read user data. Please try again.')
        setAuthLoading(false)
        return
      }
    }
    ...
    if (errParam) {
      ...
      if (msg !== 'access_denied') {          // access_denied = user cancelled, not an error
    ...
    /* Path C ── Returning user: check localStorage then verify with server */
    let cached = null
    try {
    ...
    if (cached?.login) {
      setUser(cached)       // show UI instantly (no flicker)
    }

    // Background verify — confirm cookie is still valid
    fetch('/api/auth/me', { credentials: 'include', cache: 'no-store' })
      .then(r => r.ok ? r.json() : { authenticated: false })
      .then(data => {
        if (data.authenticated && data.user?.login) {
          setUser(data.user)
          try { localStorage.setItem('sg_user', JSON.stringify(data.user)) } catch {}
        } else if (!data.authenticated) {
          setUser(null)
          try { localStorage.removeItem('sg_user') } catch {}
        }
      })
      .catch(() => { /* network error — keep cached user if we have one */ })
  ...
  const startUpload = useCallback(async () => {
    ...
    addLog(`→ Preparing ${file.name} (${fmtBytes(file.size)})`, 'info')

    try {
      const data = await uploadZip(file, p => {
        setPct(p)
        if (p === 100) {
      ...
      // Animate remaining steps as server works
      const animate = [
      ...
      for (const [delay, step, msg] of animate) {
      ...
      if (err.message.includes('authenticated') || err.message.includes('login') || err.message.includes('401')) {
        setUser(null)
        try { localStorage.removeItem('sg_user') } catch {}
      }
    }
  ...
  return (
    <>
      ...
      <div className="grid-bg" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', paddingTop: showInstall ? 48 : 0 }}>

        {/* Ambient glow */}
        <div style={{ position: 'fixed', top: '18%', left: '50%', transform: 'translate(-50%,-50%)', width: 700, height: 700, background: 'radial-gradient(circle, rgba(46,160,67,.035) 0%, transparent 68%)', pointerEvents: 'none', zIndex: 0 }} />

        {/* ── Header ──────────────────────────────────────────────────────── */}
        <header style={{ padding: '15px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', background: 'rgba(13,17,23,.88)', backdropFilter: 'blur(14px)', position: 'sticky', top: showInstall ? 48 : 0, zIndex: 100 }}>
          <Logo size={26} />
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '36px 20px 56px', position: 'relative', zIndex: 1 }}>
          ...
          {!authLoading && !user && (
            <div className="fu" style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
              <div style={{ display: 'inline-flex', padding: 20, background: 'var(--surf)', border: '1.5px solid var(--border)', borderRadius: 20, marginBottom: 28 }} className="glow">
                {I.github}
              </div>
              ...
              <p style={{ fontSize: 15, color: 'var(--muted)', lineHeight: 1.75, marginBottom: 36, maxWidth: 400, margin: '0 auto 36px' }}>
                Login with GitHub and drop any ZIP file — files are extracted and pushed to a new repository with the exact same folder structure.
              </p>
              <button className="btn btn-github btn-lg" onClick={handleLogin} disabled={loginBusy}
                style={{ minWidth: 260 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginTop: 40 }}>
                {[
                  { e: '🔒', t: 'Your Repos',    s: 'Pushes to your account' },
          ...
          {user && phase === 'idle' && (
            <div className="fu" style={{ width: '100%', maxWidth: 560 }}>
              ...
              <div className={`dz ${dragging ? 'over' : ''} ${file ? 'has-file' : ''}`}
                ...
                {!file ? (
                  ...
                  <div className="si" style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 14 }}>{fmtBytes(file.size)}</p>
              ...
              <p style={{ textAlign: 'center', fontSize: 11.5, color: 'var(--dim)', marginTop: 10 }}>
                New repo will be created under <span style={{ fontFamily: 'var(--mono)' }}>@{user.login}</span>
              </p>
            </div>
          )}
          ...
          {phase === 'uploading' && (
            <div className="fu card" style={{ width: '100%', maxWidth: 500 }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 8, textAlign: 'center', fontFamily: 'var(--mono)' }}>
                {fmtBytes(Math.round((file?.size || 0) * pct / 100))} / {fmtBytes(file?.size)}
              </p>
            </div>
          )}
              ...
              <div className="card" style={{ marginBottom: 14 }}>
                ...
                {STEPS.map((label, i) => (
                  <div key={label} className="step-row">
                    <div className={`step-dot ${stepClass(i)}`}>
                      {stepClass(i) === 'd' ? I.check : stepClass(i) === 'a' ? <Spin size={13} color="var(--green)" /> : stepClass(i) === 'e' ? I.x : i + 1}
                    </div>
                    <span style={{ fontSize: 13.5, fontWeight: stepClass(i) === 'a' ? 600 : 400, color: stepClass(i) === 'p' ? 'var(--dim)' : 'var(--text)' }}>{label}</span>
                    {stepClass(i) === 'd' && <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--green-lt)', fontFamily: 'var(--mono)' }}>done</span>}
                  </div>
                ))}
              </div>
          ...
          {phase === 'done' && result && (
            <div className="fu" style={{ width: '100%', maxWidth: 510, textAlign: 'center' }}>
              <div style={{ width: 78, height: 78, background: 'var(--green-gl)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px', border: '2px solid rgba(46,160,67,.4)' }} className="glow">
        ...
        <footer style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'center', gap: 22 }}>
          {['Privacy', 'Terms', 'Docs'].map(l => (
            <span key={l} style={{ fontSize: 12, color: 'var(--dim)', cursor: 'pointer' }}>{l}</span>
          ))}
          {user && <span style={{ fontSize: 12, color: 'var(--dim)', cursor: 'pointer' }} onClick={handleLogout}>Sign out @{user.login}</span>}
        </footer>
      </div>
    </>
  )
